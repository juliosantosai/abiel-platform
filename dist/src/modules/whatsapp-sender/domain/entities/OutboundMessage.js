...
class OutboundMessage {
    constructor({ id, empresaId, conversationId, clienteId, contenido, tipo = "TEXT", estado = "PENDING", intentos = 0, creadoEn = new Date(), enviadoEn = null }) {
        ...
        if (!clienteId)
            throw new ValidationError("El clienteId es obligatorio.", { clienteId: "required" });
        if (!contenido)
            throw new ValidationError("El contenido es obligatorio.", { contenido: "required" });
        this.id = id;
    ...
    resetearParaReintento() {
        if (!this.puedeReintentar())
            throw new DomainError("No se puede reintentar este mensaje.");
        this.estado = "PENDING";
    }
}
