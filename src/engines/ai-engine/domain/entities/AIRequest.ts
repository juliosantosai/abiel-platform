...
class AIRequest {
    ...
    iniciarProcesamiento() {
        if (this.estado !== "PENDING") throw new DomainError("Solo se puede procesar una solicitud PENDING.");
        this.estado = "PROCESSING";
    }

    completar(respuesta) {
        if (this.estado !== "PROCESSING") throw new DomainError("Solo se puede completar una solicitud PROCESSING.");
        if (!respuesta) throw new ValidationError("La respuesta es obligatoria.", { respuesta: "required" });
