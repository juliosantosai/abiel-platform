...
describe("ActivarUsuarioUseCase", () => {
    test("debe activar un usuario y publicar evento", async () => {
        ...
        const useCase = new ActivarUsuarioUseCase({
        ...
        expect(publishedEvent).toBeInstanceOf(UsuarioActivado);
    ...
    test("debe rechazar la activación si el usuario pertenece a otro tenant", async () => {
        const usuario = new Usuario({
            id: "usuario-3",
            empresaId: "empresa-a",
            nombre: "Carlos",
        ...
        await expect(useCase.execute({ id: "usuario-3", tenantContext: "empresa-b" })).rejects.toThrow(TenantError);
